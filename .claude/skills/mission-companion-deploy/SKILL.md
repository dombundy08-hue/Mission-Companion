...
- **2026-08-11 run 2 (removed the unused Community/shared-workout-program feature — button, screen, `SharedProgram`/`fetchSharedPrograms`/`postSharedProgram`/`likeSharedProgram`/`importSharedProgram`, plus the `shared_programs` table and `increment_program_likes` RPC, dropped after confirming 0 rows):** zero regressions — `programSnapshot` correctly left in place since it's also used by the unrelated `workout_programs` sync path, not sharing-specific. Audit caught one real thing worth remembering: a security-advisor WARN (leaked password protection) reported as "fixed" by a prior step turned out to still be disabled on live re-check — advisor findings tied to a dashboard-only toggle (not code/SQL this pipeline can apply) need to be re-verified with `get_advisors` at audit time, not just trusted from an earlier turn's report.
- **2026-08-11 run 3 (fixed `customScriptureCards` write-only bug — `getDeck()` in `mastery.ts` now merges the setting back into `scriptureDeck`, same pattern as the `SCRIPTURE_DEFAULTS` merge; verified live by injecting a fake pulled value and confirming it rendered under a new "My Cards" collection):** audit surfaced a real adjacent risk sitting on the exact lines just touched, not yet fixed (reported to Dom, awaiting a decision, not auto-applied per this session's scope-discipline convention): custom card ids are assigned as `maxId + 1` from currently-visible cards, so a future `SCRIPTURE_DEFAULTS` addition could reuse an id a user's custom card already claimed, silently replacing it. Folded into checklist item 4 (duplicate/id-collision check) as something to actively look for whenever a merge-by-id pattern like this one is touched again. Also confirms **write-once settings vs. growing-collection settings need different pull semantics** — `pullSettings()`'s "never clobber existing local value" guard is correct for a scalar setting but silently caps a collection setting (like `customScriptureCards`) at whatever was pulled first; not fixed this run (bigger/riskier change than the immediate bug), but worth remembering next time any `cloudSaveSetting()` call stores an array/collection rather than a scalar.
